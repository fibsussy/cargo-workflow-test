    ...
    cd "$TMP_DIR"
        if [ "$MODE" = "bin" ]; then
            if [ -n "$VERSION" ]; then
                ...
                if curl -fsSL "https://raw.githubusercontent.com/fibsussy/cargo-workflow-test/main/PKGBUILD" >/dev/null 2>&1; then
                    curl -fsSL -o PKGBUILD "https://raw.githubusercontent.com/fibsussy/cargo-workflow-test/main/PKGBUILD"
        ...
        if [ -n "$VERSION" ]; then
            ...
            if git ls-remote --tags https://github.com/fibsussy/cargo-workflow-test.git | grep -q "refs/tags/$VERSION$"; then
                git -c advice.detachedHead=false clone --branch "$VERSION" https://github.com/fibsussy/cargo-workflow-test.git repo
                cd repo
            else
                # Find newest matching version
                echo "Finding newest version matching $VERSION..."
                LATEST_TAG=$(git ls-remote --tags https://github.com/fibsussy/cargo-workflow-test.git \
                ...
                if [ -n "$LATEST_TAG" ]; then
                    ...
                    git -c advice.detachedHead=false clone --branch "$LATEST_TAG" https://github.com/fibsussy/cargo-workflow-test.git repo
                    cd repo
                else
            ...
            git clone https://github.com/fibsussy/cargo-workflow-test.git repo
            cd repo
        fi
        curl -fsSL -o PKGBUILD "https://raw.githubusercontent.com/fibsussy/cargo-workflow-test/main/PKGBUILD"
    fi
